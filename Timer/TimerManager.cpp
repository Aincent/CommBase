...
void TimerManager::AttachTimer(Safe_Smart_Ptr<Timer> &timer, int64 tick, int isNewTimer )
{
	...
	if(isNewTimer)
	{
		...
		timer->m_ticks = timer->m_start/SLOT_CHANGE_TIME;			//ʱ��Ƭ��
	}
	else
	{
		...
		timer->m_ticks = tSec/SLOT_CHANGE_TIME;			//ʱ��Ƭ��
		timer->m_bePoint += timer->m_interval;
		timer->m_tickPoint = 0;
		timer->m_elapseRound = 0;
	}

	timer->m_round = timer->m_ticks/SLOT_COUNT + 1;			//Ê±ï¿½ï¿½ï¿½ï¿½È¦ï¿½ï¿½

	GUARD(CSimLock, obj, &m_curlock);
	slotNum = (m_currSlot + (timer->m_ticks%SLOT_COUNT))%SLOT_COUNT;		//Ê±¼äÂÖ²ÛºÅ
	obj.UnLock();
