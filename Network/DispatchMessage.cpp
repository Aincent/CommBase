	...
	case Request:
		{
			...
			if((message->GetMessageID() == 1007 || message->GetMessageID() == 1008) && message->GetMessageTime() > 0)
			{
//				printf("\ndddddddddddddddddd dispatch svr time = %lld dddddddddddddddddd\n", CUtil::GetNowSecond() - message->GetMessageTime());
			}
...
//									printf("\n------------------ despatch request message[id=%d , len=%d] time[%lld] too more-----------------\n", (*it)->GetMessageID(), (*it)->GetLength(),endTime - beTime);
	...
	//				LOG_BASE(FILEINFO,"request already ack but ack timeout is null");
			...
			if(endTime - beTime > 500)
			{
//									printf("\n------------------ despatch request message[id=%d , reqid=%d, len=%d] time[%lld] too more-----------------\n", (*it)->GetMessageID(), (*it)->GetReqID(), (*it)->GetLength(),endTime - beTime);
